import{c as g,a as B}from"./assets/scroll-9bed19a6.js";import{i as c,a as f,P as D}from"./assets/vendor-ebe44317.js";const b=document.querySelector(".quote__backend"),$=new Date().toLocaleDateString();async function R(){b.innerHTML=`
    ...
    `;try{return(await f.get("https://energyflow.b.goit.study/api/quote")).data}catch{c.error({title:"Error",message:"Something went wrong, try again"})}}function S(e,t){return`<p class="quote__backend-text">${t}</p>
            <p class="quote__backend-author">${e}</p>`}function _(){if(localStorage.getItem("quote")&&localStorage.getItem("savedDate")===$){const{author:e,quote:t}=JSON.parse(localStorage.getItem("quote"));b.innerHTML=S(e,t)}else R().then(({author:e,quote:t})=>{const a={author:e,quote:t};localStorage.setItem("quote",JSON.stringify(a)),localStorage.setItem("savedDate",$),b.innerHTML=S(e,t)}).catch(e=>c.error({title:"Error",message:e.message}))}_();const O=document.getElementById("subscriptionForm"),M=document.getElementById("email"),j=new RegExp(/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/);O.addEventListener("submit",async function(e){e.preventDefault();try{j.test(M.value)?await f.post("https://energyflow.b.goit.study/api/subscription",{email:M.value}).then(t=>c.success({title:"Success",message:t.data.message})).catch(t=>c.error({title:"Error",message:t.response.data.message})):c.error({title:"Error",message:"Please enter the correct email!"})}catch{c.error({title:"Error",message:"Something went wrong, try again"})}});const w="/OlegZhdanenko/assets/exercises-sprite-d4ab7b8a.svg";function x(e,t){const a=document.getElementById("tui-pagination-container"),s=t<3?t:3,r={totalItems:e*t,itemsPerPage:e,visiblePages:s,centerAlign:!0},i=new D(a,r);return s<=1?a.style.display="none":a.style.display="block",i}const o=document.querySelector(".exercises-list");let u="Muscles",y=1;const z=12,F=9;let E,L;const v=document.getElementById("exercises-search-form"),l=document.querySelector(".exircises-category"),P=document.querySelector(".exercises-title"),Q=v.querySelector(".exercises-search-input");async function m(e,t){let a=new URLSearchParams({filter:e,page:t,limit:z});try{await f.get(`https://energyflow.b.goit.study/api/filters?${a}`).then(s=>{let r=s.data.results;E=s.data.totalPages;const i=r.map(({name:n,filter:d,imgUrl:h})=>`<li class="exercises-list-item" data-name="${n}" data-filter="${d}">
          ...
          </li>`).join("");o.insertAdjacentHTML("beforeend",i),J()}).catch(s=>{c.error({title:"Error",message:s.response.data.message})})}catch{c.error({title:"Error",message:"Something went wrong, try again"})}}await m(u,y);x(12,E).on("afterMove",({page:e})=>{o.innerHTML="",m(u,e)});function J(){let e=y;const t=document.querySelectorAll(".exercises-list-item");for(const a of t){a.addEventListener("click",s);async function s(r){const i=r.currentTarget.dataset.name,n=r.currentTarget.dataset.filter;v.classList.remove("input-hidden"),await p({filter:n,name:i,page:e}),x(9,L).on("afterMove",async({page:d})=>{o.innerHTML="",await p({filter:n,name:i,page:d})}),a.removeEventListener("click",s)}}}const k=document.querySelectorAll(".exercises-filter-button");for(const e of k)e.addEventListener("click",async function(t){v.classList.add("input-hidden");const a=t.currentTarget.dataset.name;for(const s of k)s.classList.remove("active-btn");t.target.classList.add("active-btn"),u=a,o.innerHTML="",l.dataset.name="",l.textContent="",P.textContent="Exercises",await m(a,y),x(12,E).on("afterMove",({page:s})=>{o.innerHTML="",m(u,s)})});async function p({filter:e,name:t,page:a,keyword:s=""}){const i={"Body parts":"bodypart",Muscles:"muscles",Equipment:"equipment"}[e];await f.get(`https://energyflow.b.goit.study/api/exercises?${i}=${t.toLowerCase()}&keyword=${s}&page=${a}&limit=${F}`).then(n=>{let d=n.data.results;L=n.data.totalPages,l.dataset.name=t,l.dataset.filter=e,l.textContent=g(t),P.textContent="Exercises /";const h=d.map(({bodyPart:T,burnedCalories:q,name:C,_id:H,target:I,rating:A})=>`<li class="exercises-item-page2">
