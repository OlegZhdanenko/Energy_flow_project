import{c as u,a as P}from"./assets/scroll-9bed19a6.js";import{i as v,a as x}from"./assets/vendor-ebe44317.js";const g=document.querySelector(".quote-container"),f=new Date().toLocaleDateString();async function k(){g.innerHTML=`
        <span class="loader"></span>
    `;try{return await x.get("https://energyflow.b.goit.study/api/quote").then(t=>t.data)}catch{v.error({title:"Error",message:"Something went wrong, try again"})}}function A(){if(localStorage.getItem("quote")&&localStorage.getItem("savedDate")===f){const{author:e,quote:t}=JSON.parse(localStorage.getItem("quote"));g.innerHTML=y(e,t)}else k().then(({author:e,quote:t})=>{const r={quote:t,author:e};localStorage.setItem("quote",JSON.stringify(r)),localStorage.setItem("savedDate",f),g.innerHTML=y(e,t)}).catch(e=>v.error({title:"Error",message:e.message}))}function y(e,t){return`
      ...
      `}A();const c="/OlegZhdanenko/assets/symbol-defs-b922d213.svg",p=document.querySelector(".exercises-list");p.addEventListener("click",D);const M=document.querySelector(".list-pages");M.addEventListener("click",B);const E=document.querySelector(".empty-favorite"),b=document.querySelector(".list-pages");window.addEventListener("resize",L);const l=document.querySelector(".first-page"),a=document.querySelector(".second-page"),o=document.querySelector(".third-page"),S="favorite",s=JSON.parse(localStorage.getItem(S))??[],m=s.slice(0,8),h=s.slice(8,16),T=s.slice(16,26);function L(){if(window.innerWidth<768&&s.length>=8){n(m),i(),l.classList.add("current-page");return}else{n(s),b.style.display="none";return}}L();function n(e){if(!s.length)p.innerHTML="",E.style.display="flex",b.style.display="none";else{const t=e.map(({_id:r,bodyPart:d,name:q,target:w,burnedCalories:I,time:$})=>`<li data-id="${r}" class="favopite-item">
